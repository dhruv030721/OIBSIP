...
function Card({ img, logo, name, description, price, cardKey }) {
  ...
  return (
    <div className='flex flex-col  bg-black rounded-2xl bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-30 drop-shadow-white-xl shadow-2xl border-t-orange-500 border-b-orange-500 border-2 border-x-transparent pb-5 text-white hover:-translate-y-3 transition-all h-[500px] min-w-[100%] pt-3'>
      {/* Img */}
      {steps===0 && <><div className='w-full h-[40%] rounded-xl -z-20'>
      ...
      {steps === 1 && <>
        <div className='w-full flex flex-col justify-around h-full'>
          ...
          <div className='grid grid-cols-1 gap-4 px-5'>
            <RadioButton name="Regular" radioName={cardKey} />
            <RadioButton name="Medium" radioName={cardKey} />
            <RadioButton name="Large" radioName={cardKey} />
          </div>
          <div className='flex justify-center space-x-3'>
            <BgFillButton btnName="Prev" onClick={stepDecrementHandler}/>
            <BgFillButton btnName="Next" onClick={stepIncrementHandler}/>
          </div>
        </div></>}
    </div>
  );
