...
function Card({ img, logo, name, description, price, cardKey, Ingredients, category }) {
  ...
  const AddPizzaItemHandler = () => {
    setSteps(0);
    dispatch(addCartItem({ name, crust, size, topping, price, img, category }))
    toast.success("Item Added!", { position: 'bottom-right' })
  };

  const AddBeveragesItemHandler = () => {
    dispatch(addCartItem({ name, price, category, img }))
    toast.success("Item Added!", { position: 'bottom-right' })
  }
        ...
        {steps === 3 && (
          <>
            <div className='w-full flex flex-col justify-around h-full '>
              ...
              <div className='flex justify-center space-x-3' >
                <BgFillButton btnName="Prev" onClick={stepDecrementHandler} />
                <BgFillButton btnName="Add" onClick={AddPizzaItemHandler} />
              </div>
            </div>
          </>
        )}</>) : (<>
